...
public class Fibonachchi {
    ...
    public double process(double a, double b, int n) {
        ...
        ArrayList<Double> list = FibonachchilList(n + 1);
        double u, k, l = (b - a);
        n = n - 1;
        //проверка правильности выбора n
        if (list.get(n) > (b - a) / e) {
            if (e <= (b - a) /list.get(n - 1)) {
                System.out.println(l /list.get(n));

            }

            u = (a + (list.get(n - 2)/list.get(n)) * (b - a));
            k = (a + (list.get(n - 1)/list.get(n)) * (b - a));
          ...
          //  System.out.println(" k="+k+ "   "+ list.get(n - 1)+"  "+ list.get(n));
            for (int i = 1; i < n; i++) {
                ...
                if (function(u) < function(k)) {
                    b = k;
                    k = u;
                    u = a + (list.get(n - i - 2) / list.get(n - i)) * (b - a);

                } else {
                    a = u;
                    u = k;
                    k = a + (list.get(n - i - 1) /list.get(n - i)) * (b - a);
                }
    ...
    public ArrayList<Double> FibonachchilList(int n) {
